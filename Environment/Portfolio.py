...
def random_shift(x, fraction):
    ...
    return np.clip(x * m, min_x, max_x) ## Clip (limit) the values in an array


def scale_to_start(x):
    """
    Scale pandas series so that it starts at 1.
    """
    x = (x + eps) / (x[0] + eps)
    return x
...
def sharpe(returns, freq=30, rfr=0):
    ...
    """
    return (np.sqrt(365*24/freq) * np.mean(returns - rfr + eps)) / np.std(returns - rfr + eps)
...
def max_drawdown(returns):
    ...
    max_till_now = returns[0]
    for i in range(1, len(returns)):
        returns[i] = returns[i-1] * (1 + returns[i])
        if returns[i] >= max_till_now:
            res.append(0)
            max_till_now = returns[i]
        else:
            mdd = (max_till_now - returns[i])/max_till_now
            res.append(0)
    return max(resturns)
...
class PortfolioSim(object):
    ...
    def __init__(self, asset_names=list(), steps=1*365*24, trading_cost=0.0025, time_cost=0.0):
...
#        info['market_value'] = np.cumprod([inf["rate_of_return"] for inf in self.infos + [info]])[-1]
...
#            info['rate_of_return'] = current_info['rate_of_return']
            dones[i] = done2
...
#        info['market_value'] = np.cumprod([inf["rate_of_return"] for inf in self.infos + [info]])[-1]
    ...
    def plot(self):
        ...
        df_info.set_index('date', inplace=True)
        ax = df_info[self.model_names+['market_value']].plot(title=title, rot=30, figsize = (16, 8))
        ax.grid(color='lightgrey', which="major")
        ax.legend(facecolor = 'white', edgecolor = 'lightgray')
